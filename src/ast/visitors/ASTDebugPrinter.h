...
class ASTDebugPrinter
{
    ...
    void printExprHeader(const std::string &kind, const Expression &n, const std::string &fields = "")
    {
        writeIndent();
        out << "[" << kind << "] " << n.line << ":" << n.col;
        if (!fields.empty()) out << " " << fields;
        out << " resolvedType=" << typeStr(n.resolvedType)
            << " isLvalue=" << boolStr(n.isLvalue)
    ...
    {
        if (!node) { writeIndent(); out << "<null>\n"; return; }

        if (auto x = std::dynamic_pointer_cast<Program>(node))           { visitProgram(x); return; }
        if (auto x = std::dynamic_pointer_cast<Function>(node))          { visitFunction(x); return; }
        if (auto x = std::dynamic_pointer_cast<StructDecl>(node))        { visitStructDecl(x); return; }
        if (auto x = std::dynamic_pointer_cast<VarDecl>(node))           { visitVarDecl(x); return; }

        if (auto x = std::dynamic_pointer_cast<BlockStmt>(node))         { visitBlockStmt(x); return; }
        if (auto x = std::dynamic_pointer_cast<DeclareStmt>(node))       { visitDeclareStmt(x); return; }
        if (auto x = std::dynamic_pointer_cast<ExprStmt>(node))          { visitExprStmt(x); return; }
        if (auto x = std::dynamic_pointer_cast<IfStmt>(node))            { visitIfStmt(x); return; }
        if (auto x = std::dynamic_pointer_cast<WhileStmt>(node))         { visitWhileStmt(x); return; }
        if (auto x = std::dynamic_pointer_cast<DoWhileStmt>(node))       { visitDoWhileStmt(x); return; }
        if (auto x = std::dynamic_pointer_cast<ForStmt>(node))           { visitForStmt(x); return; }
        if (auto x = std::dynamic_pointer_cast<ReturnStmt>(node))        { visitReturnStmt(x); return; }

        if (auto x = std::dynamic_pointer_cast<IntLiterals>(node))       { visitIntLiterals(x); return; }
        if (auto x = std::dynamic_pointer_cast<StringLiterals>(node))    { visitStringLiterals(x); return; }
        if (auto x = std::dynamic_pointer_cast<VariableExpr>(node))      { visitVariableExpr(x); return; }
        if (auto x = std::dynamic_pointer_cast<BinaryExpr>(node))        { visitBinaryExpr(x); return; }
        if (auto x = std::dynamic_pointer_cast<UnaryExpr>(node))         { visitUnaryExpr(x); return; }
        if (auto x = std::dynamic_pointer_cast<AssignExpr>(node))        { visitAssignExpr(x); return; }
        if (auto x = std::dynamic_pointer_cast<TernaryExpr>(node))       { visitTernaryExpr(x); return; }
        if (auto x = std::dynamic_pointer_cast<CastExpr>(node))          { visitCastExpr(x); return; }
        if (auto x = std::dynamic_pointer_cast<FunctionCallExpr>(node))  { visitFunctionCallExpr(x); return; }
        if (auto x = std::dynamic_pointer_cast<SubscriptExpr>(node))     { visitSubscriptExpr(x); return; }
        if (auto x = std::dynamic_pointer_cast<MemberExpr>(node))        { visitMemberExpr(x); return; }
        if (auto x = std::dynamic_pointer_cast<SizeOfExpr>(node))        { visitSizeOfExpr(x); return; }
        if (auto x = std::dynamic_pointer_cast<InitExpr>(node))          { visitInitExpr(x); return; }
    ...
    void visitProgram(const std::shared_ptr<Program> &n)
    {
        ...
        ++indent;
        writeIndent(); out << "nodes [" << n->nodes.size() << "]:\n";
        ++indent;
        for (const auto &tl : n->nodes) dispatch(tl);
        --indent;
        --indent;
    }
    ...
    void visitFunction(const std::shared_ptr<Function> &n)
    {
        ...
        f << "name='" << n->name << "'"
          << " type=" << typeStr(n->type)
          << " variadic=" << boolStr(n->variadic);
        printHeader("Function", *n, f.str());
        ++indent;
        writeIndent(); out << "parameters [" << n->parameters.size() << "]:\n";
        ++indent;
        for (const auto &p : n->parameters)
        {
            writeIndent();
            out << "[Parameter] " << p.line << ":" << p.col
                << " name='" << p.name << "'"
                << " type=" << typeStr(p.type) << "\n";
        }
        --indent;
    ...
    void visitStructDecl(const std::shared_ptr<StructDecl> &n)
    {
        ...
        ++indent;
        writeIndent(); out << "fields [" << n->fields.size() << "]:\n";
        ++indent;
        for (const auto &field : n->fields)
        {
            writeIndent();
            out << "[StructField] " << field.line << ":" << field.column
                << " name='" << field.name << "'"
    ...
    void visitBlockStmt(const std::shared_ptr<BlockStmt> &n)
    {
        ...
        ++indent;
        writeIndent(); out << "statements [" << n->statements.size() << "]:\n";
        ++indent;
        for (const auto &s : n->statements) dispatch(s);
        --indent;
        --indent;
    }
    ...
    void visitDeclareStmt(const std::shared_ptr<DeclareStmt> &n)
    {
        ...
        if (std::holds_alternative<std::vector<std::shared_ptr<VarDecl>>>(n->variables))
        {
            ...
            writeIndent(); out << "variables [" << vars.size() << "]:\n";
            ++indent;
            for (const auto &v : vars) dispatch(v);
            --indent;
        }
        else
        {
    ...
    void visitFunctionCallExpr(const std::shared_ptr<FunctionCallExpr> &n)
    {
        ...
        writeIndent(); out << "parameters [" << n->parameters.size() << "]:\n";
        ++indent;
        for (const auto &p : n->parameters) dispatch(p);
        --indent;
        --indent;
    }
    ...
    void visitInitExpr(const std::shared_ptr<InitExpr> &n)
    {
        ...
        ++indent;
        writeIndent(); out << "elements [" << n->elements.size() << "]:\n";
        ++indent;
        for (const auto &e : n->elements) dispatch(e);
        --indent;
        --indent;
    }
